...
void publish_message(MQTT::Client<MQTTNetwork, Countdown>* client)
{
    ...
    char buff[100];
    sprintf(buff, "%d %d %d %d", mode, message_num, ext_value[message_num - 1], gesture_index);
...
void Tilt_detection()
{
    ...
    while (1) {
        BSP_ACCELERO_AccGetXYZ(value);
        detected_angle = acos(value[1]/1000.0)*180/PI;
        uLCD.cls();
        uLCD.text_width(2);
        uLCD.text_height(2);
        uLCD.color(BLUE);
        ...
        else if (value[0] < 500 && mode == 1) {
            ext_value[message_num] = 0;
            q3.call(&publish_message, client2);
        }
